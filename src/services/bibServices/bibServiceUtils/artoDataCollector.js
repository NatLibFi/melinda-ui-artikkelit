...
import {getSubfieldValue, getSubfieldValues, getIsElectronic, getRecordType, getSourceType, isComponentPart, getPublicationYear} from './artoDataCollectorUtils.js';
...
export function collectData(jsonRecord) {
  ...
  const titles = record.get('245').flatMap(field => field.subfields.filter(sub => ['a', 'b', 'n', 'p'].includes(sub.code)).map(sub => sub.value));
  const title = titles.map(titlePart => String(titlePart).replace(titleTrimCharacters, '').replace(/\s+/ug, ' ')).join(' : ');

  const publicationInfo = collectPublicationInfo(record);
  ...
  const [f001] = record.get('001') || {'tag': '001', 'value': 'N/A'};
...
function collectPublicationInfo(record) {
  if (isComponentPart(record)) {
...
function collectHostItemData(record) {
  const [years] = getPublicationYear(record);
  const issns = getSubfieldValue(record, /^022$/u, 'a');
  const isbns = getSubfieldValue(record, /^020$/u, 'a');
  const f001 = record.get('001');
  const melindaId = `(FI-MELINDA)${f001[0].value}`;

  const [journalCountry] = record.get('008').map(field => field.value.slice(15, 18));
  const [journalLanguage] = record.get('008').map(field => field.value.slice(35, 38));
...
function collectComponentPartData(record) {
  const hostLink = record.get('773').map(field => field.subfields.filter(sub => sub.code === 'g').flatMap(sub => sub.value));
  const hostItemIds = record.get('773').map(field => field.subfields.filter(sub => sub.code === 'w').flatMap(sub => sub.value));
  const [year] = record.get('008').map(field => field.value.slice(7, 11)); // Had wrong values here. Didn't affect a thing...
...
function collectCommonData(record) {
  const notices = record.get('591').map(field => field.subfields.filter(sub => sub.code === 'd').map(sub => sub.value));
  ...
  function collectPublisherData(record) {
    // NV: There's something fundamentally wrong with this function... Simplify al lot...
...
function collectAuthorInfo(record) {
  ...
  return [...f100s, ...f110s, ...f700s, ...f710s].map(field => {
    const type = field.tag.match(/^.1.$/u) ? 'yhteisö' : 'henkilö';
    // Seriously: this should check IND1. Also original implementation is was even more iffyer...
    const subfieldsA = field.subfields.filter(sub => sub.code === 'a');
    const names = subfieldsA[0].value.split(',');
    const lastName = type === 'henkilö' ? names[0]?.trim().replace(/\.$/u, '') : '';
    const firstName = type === 'henkilö' ? names[1]?.trim().replace(/\.$/u, '') : '';
    const corporateName = type === 'yhteisö' ? names[0].trim().replace(/\.$/u, '') : '';
    ...
    if (!lastName && !firstName && !corporateName) {
      return false;
    }
