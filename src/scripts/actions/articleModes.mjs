...
import {collectRawFormData} from '/scripts/actions/articleCollectFormData.mjs';
import {activateTab} from '/scripts/artikkelit.mjs';
/* Shared imports */
import {disableElement, enableElement, getAllDescendants, isHidden, isVisible} from '/shared/scripts/elements.js';
...
window.showArticleFormReadMode = function (event = undefined) {
  ...
  disableElement(highlightErrorsButton);
  ...
  function fillPreview(formData) {
    ...
    fill(formData.collecting?.f598a, 'f598aPreview');
...
window.showArticleFormEditMode = function (event = undefined) {
  ...
  enableElement(highlightErrorsButton);
...
export function showFormActionsOnEdit() {
  ...
  const newEmptyRecordButton = document.getElementById('emptyRecordButton');
  //const newCopyRecordButton = document.getElementById('copyRecordButton');
...
export function showFormActionsAfterSave(melindaId, testServer) {
  const divActionsOnEdit = document.getElementById('actionsOnEdit');
  const showEditModeButton = document.getElementById('formEditMode');
  const showReadModeButton = document.getElementById('formReadMode');
  const clearFormButton = document.getElementById('formClear');
  ...
  const melindaById = testServer ? 'https://melinda-test.kansalliskirjasto.fi/byid' : 'https://melinda.kansalliskirjasto.fi/byid';
  formNotes.innerHTML = `Luotu tietue Melindassa: <a href="${melindaById}/${melindaId}">${melindaById}/${melindaId}</a><br><br>Aloita uuden tietueen kuvailu tyhjältä lomakkeelta tai vanhan tiedot mukaan ottaen, tai siirry editoimaan tallentamaasi tietuetta.`;
  ...
  if (editRecordButton) {
    editRecordButton.onclick = function() {
      ...
      // Change the tab HERE
      const editorTab = document.getElementById('panelTabEditor');
      activateTab(editorTab); // Activates tab, not the page...
    }
  }
...
// when form is locked and the record is saved
export function showRecordActionsAfterSave(melindaId = undefined) {
  ...
  recordNotes.innerHTML = `Kuvailtu artikkeli tallennettiin ARTO-kokoelmaan${melindaId ? ` id:llä ${melindaId}` : ''}`;
