...
/* Local imports */
import {validateForm} from '/scripts/actions/articleValidateForm.mjs';
import {idbGet} from '/scripts/indexedDB.mjs';
import {validateArticleRecord} from '/scripts/restCalls.mjs'

/* Shared imports */
import {enableElement, highlightElement} from '/shared/scripts/elements.js';
  ...
  function formCheckFailed() {
    ...
    function getErrorNote() {
      ...
      formErrors.forEach((error) => {
        ...
        if (error.errorType === 'field591') {
          formattedErrors = formattedErrors + `<li>Osio <b>${error.fieldsetLegend}</b> => kenttä <b>${error.label}</b> (591\$a) tarvitaan, jotta tieteenala (591\$h\$i) voidaan lisätä</li>`;
        }
...
//   - then call function validateArticleRecord and pass the record as parameter
