/*****************************************************************************/
/* FORM INTERFACE FOR SEARCHING PUBLICATION FOR REVIEW                       */
/*****************************************************************************/
...
import {idbAddValueToLastIndex, idbClear, idbDel, idbGet, idbSet, idbGetStoredValues} from '/scripts/indexedDB.mjs';
import {getPublicationByMelindaId, getPublicationByISBN, getPublicationByISSN, getPublicationByTitle} from '/scripts/restCalls.mjs';
import {createIconButton, createP, formToJson, setOptions, sortRecordData} from '/scripts/utils.mjs';
...
function searchResultChange(event) {
  if (event.target.value !== '') {
    idbGet('artoTempReviews', parseInt(event.target.value)).then(data => {
      ...
      document.getElementById('teoksen-melindaId').innerHTML = data.melindaId.replace('(FI-MELINDA)', '');
...
function addReview(event) {
  ...
  idbGet('artoTempReviews', parseInt(reviewIndex)).then(tempReview => {
    ...
    idbGetStoredValues('artoReviews').then(reviews => {
      // Removed title from condition. See MUU-793 for details. We boldly assume, that checking first id is enough.
      if (reviews.some(review => review.melindaId === tempReview.melindaId)) {
        showSnackbar({style: 'alert', text: 'Artikkelille on jo lisätty tämä arvosteltu teos'});
        return;
      }
...
export function refreshReviewsList(skipUpdate = false) {
...
function clearReviews(event) {
  event.preventDefault();
  idbClear('artoReviews').then(() => refreshReviewsList(true));
}
...
function refreshSearchResultSelect() {
  ...
  idbGetStoredValues('artoTempReviews').then(sources => {
    const data = sources.map(record => {
      const {title} = record;
      const melindaId = record.melindaId;
      ...
      const yearsTogether = ` (${yearsStart}${hyphen}${yearsEnd})`;

      const text = `${title}${yearsTogether === ' ()' ? '' : yearsTogether}, MELINDA-ID: ${melindaId}`;
