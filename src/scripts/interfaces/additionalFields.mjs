/*****************************************************************************/
/* FORM INTERFACE FOR ADDITIONAL FIELDS                                      */
/* - NOTES                                                                   */
/* - OTHER TITLES                                                            */
/* - UDKs                                                                    */
/* - OTHER RATINGS                                                           */
...
export function initAdditionalFields() {
  ...
  document.getElementById('lisakentat-yleinen-huomautus').addEventListener('input', noteCharacterCounter);
  document.getElementById('yleinen-huomautus-lisaa-form').addEventListener('reset', resetNoteCharacterCounter);

  refreshNotesList(false);
  refreshOtherTitlesList(false);
  refreshUDKsList(false);
  refreshOtherRatingsList();
}

export function refreshNotesList(skipUpdate = false) {
...
export function refreshUDKsList(skipUpdate = false) {
...
export function refreshOtherTitlesList(skipUpdate = false) {
...
export function refreshOtherRatingsList(skipUpdate = false) {
  ...
  idbGetStoredValues('artoOtherRatings').then(otherRatings => {
    otherRatings.forEach(otherRatingData => {
      if (!otherRatingData.a084 || !otherRatingData.two084) {
        // 084: both $a and $2 are required subfields
        return;
      }
...
export function addOtherRating(event) {
  ...
  if (data.two084 === '') {
    showSnackbar({style: 'alert', text: 'Muun luokituksen lähde (084 $2) ei voi olla tyhjä'});
    return;
  }
...
export function noteCharacterCounter(event) {
  const maxLength = 5000;
  const currentLength = event.target.value.length || 0;
  document.getElementById('yleinen-huomautus-merkkiraja').innerHTML = `${currentLength}/${maxLength}`;
  const warning = document.getElementById('yleinen-huomautus-merkkirajan-ylitys');
  if (currentLength > maxLength) {
    const overLimitText = event.target.value.slice(maxLength);
    warning.innerHTML = `<strong>HUOM!</strong> Huomautuksen merkkiraja on ylittynyt.<br><br>
                          Voit jatkaa kirjoittamista ja lisätä huomautuksen, mutta rajan ylittävä osuus katkaistaan ennen tallennusta Melindaan.<br><br>
...
export function resetNoteCharacterCounter(event) {
  document.getElementById('yleinen-huomautus-merkkiraja').innerHTML = '0/5000';
  const warning = document.getElementById('yleinen-huomautus-merkkirajan-ylitys');
  warning.innerHTML = '';
  warning.style.padding = '';
}
...
window.resetNote = (event) => {
  event.preventDefault();
  const textbox = document.getElementById('lisakentat-yleinen-huomautus');
