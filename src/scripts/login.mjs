...
window.initialize = function () {
  console.log('Initializing Artikkelit login');
  addSubmitLoginFormEventListener();
}

function addSubmitLoginFormEventListener() {
  const loginForm = document.getElementById('loginForm');
  loginForm.addEventListener('submit', logIntoArtikkelit);
}

async function logIntoArtikkelit(event) {
  console.log('Submit login form event: log into Artikkelit');
  eventHandled(event);
